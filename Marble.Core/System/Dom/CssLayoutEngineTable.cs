	...
	internal sealed class CssLayoutEngineTable
	{
				...
				foreach (CssBox row in _allRows)
				{
					...
					for (int i = 0; i < _columnCount; i++)
					{
						if (i < 20 || double.IsNaN(_columnWidths[i])) // limit column width check
						{
							...
							{
								double len =
									CssValueParser.ParseLength(row.Boxes[i].Width, availCellSpace, row.Boxes[i]);
			...
			else
			{
				...
				for (int i = 0; i < _columnWidths.Length; i++)
				{
					if (maxFullWidths[i] > _columnWidths[i])
					{
						...
						_columnWidths[i] =
							Math.Min(
								_columnWidths[i] + (availCellSpace - occupedSpace) /
								Convert.ToSingle(_columnWidths.Length - i), maxFullWidths[i]);
		...
		private void LayoutCells(RGraphics g)
		{
			...
			if (_tableBox.TextAlign == CssConstants.Center || _tableBox.TextAlign == CssConstants.Right)
			{
				...
				_tableBox.Location =
					new RPoint(
						startx - _tableBox.ActualBorderLeftWidth - _tableBox.ActualPaddingLeft - GetHorizontalSpacing(),
			...
			_tableBox.ActualBottom =
				Math.Max(maxBottom, starty) + GetVerticalSpacing() + _tableBox.ActualBorderBottomWidth;
		}
		...
		private double GetCellWidth(int column, CssBox b)
		{
			...
			return
				sum; // -b.ActualBorderLeftWidth - b.ActualBorderRightWidth - b.ActualPaddingRight - b.ActualPaddingLeft;
		}
		...
		private void GetColumnsMinMaxWidthByContent(bool onlyNans, out double[] minFullWidths,
			out double[] maxFullWidths)
		{
		...
		{
			return GetAvailableTableWidth() - GetHorizontalSpacing() * (_columnCount + 1) -
			       _tableBox.ActualBorderLeftWidth - _tableBox.ActualBorderRightWidth;
		}
