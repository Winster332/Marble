...
using Marble.Core.System.Handlers;
using Marble.Core.System.Parse;
using Marble.Core.System.Utils;

namespace Marble.Core.System.Dom
{
	/// <summary>
	...
	public class CssBox : CssBoxProperties, IDisposable
	{
					...
					if (endIdx > startIdx)
					{
						if (preserveSpaces)
							_boxWords.Add(new CssRectWord(this,
								HtmlUtils.DecodeHtml(_text.Substring(startIdx, endIdx - startIdx)), false, false));
					}
					else
					{
						endIdx = startIdx;
						...
						if (endIdx > startIdx)
						{
							var hasSpaceBefore = !preserveSpaces &&
							                     (startIdx > 0 && _boxWords.Count == 0 &&
							                      char.IsWhiteSpace(_text[startIdx - 1]));
							var hasSpaceAfter = !preserveSpaces &&
							                    (endIdx < _text.Length && char.IsWhiteSpace(_text[endIdx]));
							_boxWords.Add(new CssRectWord(this,
								HtmlUtils.DecodeHtml(_text.Substring(startIdx, endIdx - startIdx)), hasSpaceBefore,
								hasSpaceAfter));
						}
					}
		...
		protected virtual void PerformLayoutImp(RGraphics g)
		{
			...
			if (IsBlock || Display == CssConstants.ListItem || Display == CssConstants.Table ||
			    Display == CssConstants.InlineTable || Display == CssConstants.TableCell)
			{
				...
				if (Display != CssConstants.TableCell)
				{
					...
					{
						left = ContainingBlock.Location.X + ContainingBlock.ActualPaddingLeft + ActualMarginLeft +
						       ContainingBlock.ActualBorderLeftWidth;
						top = (prevSibling == null && ParentBox != null ? ParentBox.ClientTop :
		...
		private void CreateListItemBox(RGraphics g)
		{
			if (Display == CssConstants.ListItem && ListStyleType != CssConstants.None)
			{
				if (_listItemBox == null)
				{
					...
					{
						_listItemBox.Text =
							new SubString(GetIndexForList().ToString(CultureInfo.InvariantCulture) + ".");
					}
					else if (ListStyleType.Equals(CssConstants.DecimalLeadingZero,
						StringComparison.InvariantCultureIgnoreCase))
					{
						_listItemBox.Text =
							new SubString(GetIndexForList().ToString("00", CultureInfo.InvariantCulture) + ".");
					}
					else
					{
						_listItemBox.Text =
							new SubString(CommonUtils.ConvertToAlphaNumber(GetIndexForList(), ListStyleType) + ".");
					}
		...
		internal double GetMinimumWidth()
		{
			...
			if (maxWidthWord != null)
			{
				...
				while (box != null)
				{
					padding += box.ActualBorderRightWidth + box.ActualPaddingRight + box.ActualBorderLeftWidth +
			...
			ref double marginSum)
		{
			...
			// add the padding 
			paddingSum += box.ActualBorderLeftWidth + box.ActualBorderRightWidth + box.ActualPaddingRight +
			              box.ActualPaddingLeft;
		...
		protected double MarginTopCollapse(CssBoxProperties prevSibling)
		{
			...
			else if (_parentBox != null && ActualPaddingTop < 0.1 && ActualPaddingBottom < 0.1 &&
			         _parentBox.ActualPaddingTop < 0.1 && _parentBox.ActualPaddingBottom < 0.1)
			{
				value = Math.Max(0,
					ActualMarginTop - Math.Max(_parentBox.ActualMarginTop, _parentBox.CollapsedMarginTop));
			}
			else
			{
			...
			if (Width.Length > 0)
			{
				...
				foreach (var word in Words)
				{
					if (!word.IsLineBreak)
					{
						...
						if (clip != RRect.Empty)
						{
							var wordPoint = new RPoint(word.Left + offset.X, word.Top + offset.Y + word.Height);
							if (word.Selected)
							{
								// handle paint selected word background and with partial word selection
								var wordLine = DomUtils.GetCssLineBoxByWord(word);
								var left = word.SelectedStartOffset > -1
									? word.SelectedStartOffset
									: (wordLine.Words[0] != word && word.HasSpaceBefore ? -ActualWordSpacing : 0);
								var padWordRight = word.HasSpaceAfter && !wordLine.IsLastSelectedWord(word);
								var width = word.SelectedEndOffset > -1
									? word.SelectedEndOffset
									: word.Width + (padWordRight ? ActualWordSpacing : 0);
								var rect = new RRect(word.Left + offset.X + left, word.Top + offset.Y, width - left,
									wordLine.LineHeight);

								g.DrawRectangle(GetSelectionBackBrush(g, false), rect.X, rect.Y, rect.Width,
									rect.Height);
								    ...
								    (word.SelectedStartOffset > 0 || word.SelectedEndIndexOffset > -1))
								{
									...
									g.DrawString(word.Text, ActualFont, ActualColor, wordPoint,
										new RSize(word.Width, word.Height), isRtl);
									g.PopClip();
									g.PushClip(rect);
									g.DrawString(word.Text, ActualFont, GetSelectionForeBrush(), wordPoint,
										new RSize(word.Width, word.Height), isRtl);
									g.PopClip();
								}
								else
		...
		public override string ToString()
		{
			...
			{
				return string.Format("{0}{1} {2}: {3}", ParentBox == null ? "Root: " : string.Empty, tag, Display,
					Text);
			}
		}
