	...
	internal sealed class CssBoxFrame : CssBox
	{
		...
		private void LoadYoutubeDataAsync(Uri uri)
		{
			ThreadPool.QueueUserWorkItem(state =>
			{
				...
				{
					HtmlContainer.ReportError(HtmlRenderErrorType.Iframe, "Failed to get youtube video data: " + uri,
						ex);
					...
					if (e.Error == null)
					{
						...
						if (idx > -1)
						{
							...
							if (idx > -1)
							{
								...
								if (idx > -1)
								{
									...
									if (endIdx > -1)
									{
										_videoTitle = e.Result.Substring(idx + 1, endIdx - idx - 1)
											.Replace("\\\"", "\"");
									}
								}
							}
						...
						if (idx > -1)
						{
							...
							if (iidx > -1)
							{
								...
								if (endIdx > -1)
								{
									_videoImageUrl = e.Result.Substring(iidx, endIdx - iidx).Replace("\\\"", "\"")
										.Replace("\\", "");
								}
							}
						}
		...
		private void DrawTitle(RGraphics g, RRect rect)
		{
			if (_videoTitle != null && _imageWord.Width > 40 && _imageWord.Height > 40)
			{
				...
				g.DrawRectangle(g.GetSolidBrush(RColor.FromArgb(160, 0, 0, 0)), rect.Left, rect.Top, rect.Width,
					ActualFont.Height + 7);
		...
		private bool IsLayoutRequired()
		{
			...
			return (width.Number <= 0 || width.Unit != CssUnit.Pixels) ||
