...
namespace Marble.Core.System.Utils
{
	...
	internal static class CommonUtils
	{
		...
		{
			"あ", "ぃ", "ぅ", "ぇ", "ぉ", "か", "き", "く", "け", "こ", "さ", "し", "す", "せ", "そ", "た", "ち", "つ", "て", "と", "な",
			"に", "ぬ", "ね", "の", "は", "ひ", "ふ", "へ", "ほ", "ま", "み", "む", "め", "も", "ゃ", "ゅ", "ょ", "ら", "り", "る", "れ",
			"ろ", "ゎ", "ゐ", "ゑ", "を", "ん"
		};
		...
		public static FileInfo GetLocalfileName(Uri imageUri)
		{
			...
			{
				validFileName = validFileName.Substring(0, 24) + validFileName.Substring(24).GetHashCode() +
		...
		public static string ConvertToAlphaNumber(int number, string style = CssConstants.UpperAlpha)
		{
			...
			else if (style.Equals(CssConstants.Hiragana, StringComparison.InvariantCultureIgnoreCase) ||
			         style.Equals(CssConstants.HiraganaIroha, StringComparison.InvariantCultureIgnoreCase))
			{
				return ConvertToSpecificNumbers2(number, _hiraganaDigitsTable);
			}
			else if (style.Equals(CssConstants.Katakana, StringComparison.InvariantCultureIgnoreCase) ||
			         style.Equals(CssConstants.KatakanaIroha, StringComparison.InvariantCultureIgnoreCase))
			{
				return ConvertToSpecificNumbers2(number, _satakanaDigitsTable);
			}
			else
			{
				var lowercase = style.Equals(CssConstants.LowerAlpha, StringComparison.InvariantCultureIgnoreCase) ||
				                style.Equals(CssConstants.LowerLatin, StringComparison.InvariantCultureIgnoreCase);
