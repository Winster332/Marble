...
namespace Marble.Core.System.Handlers
{
	...
	internal sealed class ContextMenuHandler : IDisposable
	{
		...
		private void OnOpenLinkClick(object sender, EventArgs eventArgs)
		{
			try
			{
				_currentLink.HtmlContainer.HandleLinkClicked(_parentControl, _parentControl.MouseLocation,
					_currentLink);
			}
		...
		private void OnCopyImageLinkClick(object sender, EventArgs eventArgs)
		{
			...
			{
				_htmlContainer.ReportError(HtmlRenderErrorType.ContextMenu, "Failed to copy image url to clipboard",
					ex);
			}
			finally
			{
