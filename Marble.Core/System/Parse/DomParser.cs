...
namespace Marble.Core.System.Parse
{
	...
	internal sealed class DomParser
	{
			...
			foreach (var selector in block.Selectors)
			{
				...
				while (!matched)
				{
					...
					if (!matched && box.HtmlTag.HasAttribute("class"))
					{
						...
						if (selector.Class.Equals("." + className, StringComparison.InvariantCultureIgnoreCase) ||
						    selector.Class.Equals(box.HtmlTag.Name + "." + className,
		...
		private static void ApplyTableBorder(CssBox table, string border)
		{
			...
			{
				cell.BorderLeftStyle = cell.BorderTopStyle =
					cell.BorderRightStyle = cell.BorderBottomStyle = CssConstants.Solid;
		...
		private static void CorrectBlockSplitBadBox(CssBox parentBox, CssBox badBox, CssBox leftBlock)
		{
			...
			else if (splitBox.ParentBox != null && parentBox.Boxes.Count > 1)
			{
				...
				if (splitBox.HtmlTag != null && splitBox.HtmlTag.Name == "br" &&
