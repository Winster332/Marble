...
namespace Marble.Core.System.Parse
{
	...
	internal static class HtmlParser
	{
		...
		private static bool ParseHtmlTag(string source, int idx, int length, out string name,
		...
		/// <param name="attributes">return the dictionary of tag attributes</param>
		private static void ExtractAttributes(string source, int idx, int length,
			out Dictionary<string, string> attributes)
		{
			attributes = null;
			...
			while (startIdx < idx + length)
			{
				...
				if (startIdx < idx + length)
				{
					...
					if (startIdx < idx + length)
					{
						...
						while (endIdx < idx + length &&
						       (hasPChar ? source[endIdx] != pChar : !char.IsWhiteSpace(source, endIdx)))
							endIdx++;
