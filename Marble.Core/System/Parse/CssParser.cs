...
namespace Marble.Core.System.Parse
{
	/// <summary>
	...
	internal sealed class CssParser
	{
		...
		private CssBlock ParseCssBlockImp(string className, string blockSource)
		{
			...
			if (!string.IsNullOrEmpty(className) &&
			    (psedoClass == null || psedoClass == "link" || psedoClass == "hover"))
			{
				string firstClass;
		...
		/// <param name="propValue">the value of the css property to add</param>
		/// <param name="properties">the properties collection to add to</param>
		private static void ParseLengthProperty(string propName, string propValue,
		...
		private void ParseFontProperty(string propValue, Dictionary<string, string> properties)
		{
			int mustBePos;
			string mustBe =
				RegexParserUtils.Search(RegexParserUtils.CssFontSizeAndLineHeight, propValue, out mustBePos);
		...
		private static string ParseImageProperty(string propValue)
		{
			...
			if (startIdx > -1)
			{
				...
				{
					endIdx -= 1;
					while (startIdx < endIdx && (char.IsWhiteSpace(propValue[startIdx]) ||
					                             propValue[startIdx] == '\'' || propValue[startIdx] == '"'))
						startIdx++;
		...
		private string ParseFontFamilyProperty(string propValue)
		{
			...
			{
				while (char.IsWhiteSpace(propValue[start]) || propValue[start] == ',' || propValue[start] == '\'' ||
				       propValue[start] == '"')
					start++;
		...
		/// </summary>
		private static void SplitMultiDirectionValues(string propValue, out string left, out string top,
